...
export const calculateWheelCost = (config) => {
  ...
  return (
    4 * (Math.PI * config.wheel_rad * config.wheel_rad) +
    10 * config.wheel_width
  );
};
...
export const octagonArea = (vertices) => {
  ...
  return (
    trapezoidArea(t1verts) + trapezoidArea(t2verts) + trapezoidArea(centerverts)
  );
};
...
export const calculateBodyWeight = (config) => {
  ...
  return (
    0.125 *
    areas.map((h, i) => h * config.hull_densities[i]).reduce((a, h) => a + h)
  );
};
