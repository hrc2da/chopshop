...
import {
  incrementTestDriveVideoCounter,
  decrementTestDriveVideoCounter,
} from "../actions/testDrive";

function mapStateToProps(state) {
  return {
    modal_state: state.tabs ? state.tabs.video_modal : true,
    ...
    design_plan: state.session? state.session.driver_design_plan : "",
    static_url: state.carRacingStaticUrl,
    api_url: state.carRacingApiUrl,
    numTestDrives:
      state.session && state.session.initial_test_drives
        ? state.session.initial_test_drives.length
        : 0,
  };
}
