...
function mapStateToProps(state, ownProps) {
  ...
  let scale = 2.0;
  let left_bound = document.getElementById("workspace")
    ? document.getElementById("workspace").getBoundingClientRect().left
    : 0;
  // console.log("untransformed coords",state.carConfig[ownProps.polygon][ownProps.index])
  let transformedCoords = transform(
    [state.carConfig[ownProps.polygon][ownProps.index]],
    xOffset,
    yOffset,
    scale
  );
...
function mapDispatchToProps(dispatch, ownProps) {
  return {
    handleMove: (e, x, y, xOffset, yOffset, scale) => {
      ...
      dispatch(
        setConfigVar(HULL_VERTEX, {
          polygon: ownProps.polygon,
