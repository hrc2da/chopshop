...
export const deselectCompCar = () => {
  return (dispatch, getState) => {
    ...
    let testedCars = session["tested_designs"];
    let currentCar = testedCars[testedCars.length - 1];
    dispatch(setTestDriveVideo(session.tested_videos[testedCars.length - 1]));
...
export const selectCompCarSession = (type,index) => {
  return (dispatch, getState) => {
    let session = getState().session;
    dispatch(setCompCarConfig(session.tested_designs[index]));
    dispatch(setTestDriveVideo(session.tested_videos[index]));
    return;
  }
}