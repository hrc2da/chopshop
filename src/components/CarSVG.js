...
export const transform = (
  ...
  return coords.map((v) => [
    parseInt(scale * v[0] + xOffset),
    parseInt(scale * v[1] + yOffset),
  ]);
};

export const invtransform = (
  coords,
  xOffset,
  yOffset,
  scale = 2.0,
...
class CarSVG extends Component {
  ...
  render() {
    ...
    let wheel_coords = wheels
      ? wheels.map((w) =>
          this.wheelattrs2Coords(
            this.props.config.wheel_rad,
            this.props.config.wheel_width,
            w
          )
        )
      : [];
    ...
    let bumperCoords = bumper
      ? clockwiseSort(transform(bumper, 0, yOffset + 30, 2.0))
      : undefined;
    let hull1Coords = hull1
      ? clockwiseSort(transform(hull1, xOffset + 175, 0, 2.0))
      : undefined;
    let SVGResult;
    ...
    if (wheels) {
      ...
      let minWheelPosY = wheels.reduce(
        (a, w) => Math.min(a, w[1]),
        maxWheelPosY
      );
      ...
      wheelPairs = [frontPair, rearPair].map((pair) =>
        transform(pair, xOffset, yOffset, 2.0)
      );
    }
    ...
    if (this.widthChanged && this.lengthChanged) {
      SVGResult = (
        <React.Fragment>
          ...
          {wheels && (
            <line
              x1={wheelPairs[0][0][0] + this.props.config.wheel_width / 2}
              y1={wheelPairs[0][0][1]}
              x2={wheelPairs[0][1][0] - this.props.config.wheel_width / 2}
              y2={wheelPairs[0][1][1]}
              strokeWidth={5}
          ...
          {wheels && (
            <line
              x1={wheelPairs[1][0][0] + this.props.config.wheel_width / 2}
              y1={wheelPairs[1][0][1]}
              x2={wheelPairs[1][1][0] - this.props.config.wheel_width / 2}
              y2={wheelPairs[1][1][1]}
              strokeWidth={5}
              stroke={this.props.wheelColor}
            />
          ...
          {bumper &&
            this.coords2SVG(
              bumper,
              this.props.hullColor,
              this.props.config.hull_densities[0],
              xOffset,
              yOffset
            )}
          {hull1 &&
            this.coords2SVG(
              hull1,
              this.props.hullColor,
              this.props.config.hull_densities[1],
              xOffset,
              yOffset
            )}
          {hull2 &&
            this.coords2SVG(
              hull2,
              this.props.hullColor,
              this.props.config.hull_densities[2],
              xOffset,
              yOffset
            )}
          {spoiler &&
            this.coords2SVG(
              spoiler,
              this.props.hullColor,
              this.props.config.hull_densities[3],
              xOffset,
              yOffset
            )}
          {wheel_coords.map((w) =>
            this.coords2SVG(
              w,
              this.props.wheelColor,
              0.6 + (0.25 * this.props.config.friction_lim) / 1e4,
              xOffset,
              yOffset
            )
          )}
          ...
          //SHADOW OF LENGTH TEXT
          {hull1 && (
            <text x={hull1Coords[1][0] - 75} y={yOffset} fill="yellow">
              {carLength} m
            </text>
          )}{" "}
          //ACTUAL LENGTH TEXT
          ...
          //SHADOW OF WIDTH TEXT
          {bumper && (
            <text x={xOffset - 20} y={bumperCoords[0][1]} fill="yellow">
              {carWidth} m
            </text>
          )}{" "}
          //ACTUAL WIDTH TEXT
      ...
      SVGResult = (
        <React.Fragment>
          ...
          //ACTUAL LENGTH TEXT //WIDTH
          {bumper && (
            <text x={xOffset - 20} y={bumperCoords[0][1]} fill="black">
              {carWidth} m
            </text>
          )}
        </React.Fragment>
      ...
      SVGResult = (
        <React.Fragment>
          ...
          {/* //LENGTH */}
          {hull1 && (
            <text x={hull1Coords[1][0] - 75} y={yOffset} fill="black">
              {carLength} m
            </text>
          )}
          {/* //WIDTH */}
