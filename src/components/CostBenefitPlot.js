...
let CostBenefitPlot = ({
  ...
  if (gaCars.length > 0 && false) {
    //taking the GA cars out
    cars.push({
  ...
  if (testedCars.length > 0) {
    cars.push({
      ...
      data: testedCars.map((car, index) => {
        // let cost = car['result'][1]+calculateCarCost(car['config']);
        // let benefit = car['result'][0]-car['result'][2]
        let cost = calculateCarCost(car);
        let benefit = testedRewards[index];
        return { id: index, x: benefit, y: cost };
      }),
    ...
    <div style={{ height: "220px" }}>
      <ul style={{ listStyle: "none" }}>
        <li>
          <div
            style={{
              backgroundColor: "rgba(192,1,1,1)",
              height: "10px",
              width: "10px",
      ...
      <Scatter
        ...
        options={{
          ...
          legendCallback: (chart) => {
            ...
            text.push(
              '<li><span style="background-color: rgba(192,1,1,1)"></span>'
            );
